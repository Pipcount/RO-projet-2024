...
def three_opt_parallel(segments, best_solution, best_distance, start_time, shape, solution_lock, barrier, event):
    ...
    while time.time() - start_time < time_limit:
        ...
        for i, j, k in segments:
            ...
            for swap in [three_opt_swap1]:#, three_opt_swap2, three_opt_swap3, three_opt_swap4, three_opt_swap5, three_opt_swap6, three_opt_swap7]:
                ...
                new_distance = total_distance(new_solution, data, best_distance_for_now)
        ...
        four_opt = False
        with best_distance.get_lock():
            if best_distance.value == best_distance_for_now:
                print(f"3-opt did not find a better solution, trying 4-opt, time: {time.time() - start_time}")
                four_opt = True

        if four_opt:
            for i, j, k in segments:
                if time.time() - start_time > time_limit or event.is_set():
                    break
                ls = [l for l in range(k + 1, len(best_solution_for_now) - 1)]
                np.random.shuffle(ls)
                for l in ls:
                    if time.time() - start_time > time_limit or event.is_set():
                        break
                    new_solution = four_opt_swap1(best_solution_for_now, i, j, k, l)
                    new_distance = total_distance(new_solution, data, best_distance_for_now)
                    if new_distance < best_distance_for_now:
                        print(f"Process {os.getpid()} found a better solution: {new_distance}, delta: {best_distance_for_now - new_distance}, time: {time.time() - start_time}")
                        if event.is_set():
                            break
                        event.set()
...
def three_opt(solution):
    ...
    for chunk in chunks:
        process = multiprocessing.Process(target=three_opt_parallel, args=(chunk, best_solution, best_distance_value, start, solution_shape, best_solution_lock, barrier, event))
        process.start()

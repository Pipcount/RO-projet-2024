...
"""
Ce fichier contient l'implémentation de l'algorithme 3-opt pour le problème du Pays de Cholet, avec le bonus multi-commodité.
...
def verify_calculate_weight(solution : np.ndarray, data : dict) -> bool:
    ...
    Returns:
        bool: True si la solution respecte les contraintes de poids, False sinon.
    """
...
def verify_calculate_weight_indexed(solution : np.ndarray, weights : np.ndarray, index : int) -> bool:
    """
    Vérifie si la solution respecte les contraintes de poids pour une certaine commodité et renvoie True si c'est le cas, False sinon.

    Args:
        solution (np.ndarray): La solution à vérifier.
        weights (np.ndarray): Les poids des déchets aux noeuds.
        index (int): L'index de la commodité à vérifier.
...
if __name__ == "__main__":
    ...
    weights = data["bonus_multi_commodity_Cholet_pb1_bis.pickle"]
    # Les poids des deux derniers nœuds valent -5850 pour représenter l'exutoire
    weights[-1] = (-5850, -5850)
    weights[-2] = (-5850, -5850)
